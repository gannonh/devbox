...
* **init guidance**: `init` now prints a post-create customization guide pointing at the repo-specific surfaces (`post-create.sh` hook, `devcontainer.json` ports/env, explicit `--env` secrets, `provision.sh` agent switching, `Dockerfile` apt packages) before the boot prompt, rather than implying every generated file needs editing. Extended spec criterion 1 to cover the guide.
...
* **Pinned SDK contract**: `@vercel/sandbox` is pinned to exactly `3.1.0` in both `package.json` and the lockfile, and the client boundary asserts the exact `update({ ports })` request and full-list replacement semantics.
* **The port maximum is 14, measured**: The live check the ticket required found three disagreeing sources. The installed 3.1.0 declaration comments "up to 15 ports"; the request schema refuses a 16th with `` `ports` should NOT have more than 15 items ``; and an update carrying exactly 15 fails with an opaque 500 every time, with any port values. Devbox caps the full set at 14 (13 app ports plus `6080`) so an over-large set fails with an actionable message instead of a 500. Reproduce with `scripts/vercel/app-port-uat.mjs`.
* **Two behaviors the UAT surfaced**: a port update regenerates every route's `*.vercel.run` subdomain, so a URL copied before an update goes stale — devbox re-reads routes after updating and always prints current URLs. The relay now rewrites only the app's `Host` to `localhost:<port>`, so Vite's host check accepts the ordinary loopback-bound command while `Origin`, cookies, and `Authorization` remain application-owned.
...
* **Public app routes work with the ordinary dev command**: Issue [#17](https://github.com/gannonh/devbox/issues/17) — devbox no longer exposes the app's own listener. Each confirmed logical app port gets one small fixed-target process inside the Sandbox that forwards HTTP, streaming HTTP, and WebSocket traffic to `localhost:<port>`, and the public route points at *that* listener. A loopback-bound dev server is reachable without `--host`, and Vite 5.4.12+ sees a `Host` it accepts without `server.allowedHosts`. Accepted [ADR 0007](./adrs/0007-relay-backed-public-app-routes.md), which retires the "add these two settings yourself" consequence of ADR 0005.
* **Nothing app-shaped is created public**: a fresh Sandbox is created with `6080` only. Configured `forwardPorts` are still parsed and validated at create time, but they are logical app ports, not listeners, so they reach the route set only as relays after the existing confirmation. One app still costs one exposed slot, so the measured 14-route ceiling is unchanged.
* **The header contract is the interesting part**: `Host` is rewritten to `localhost:<port>` — that is the compatibility decision — while `Origin`, cookies, and `Authorization` pass through untouched, so the app's own origin policy still decides. Client-supplied `Forwarded`/`X-Forwarded-*` values are dropped and replaced with what the relay observed. Nothing in a request can select the upstream: origin-form targets only, `400` on a malformed or absolute-form authority, and a hard refusal to target `5900`/`6080`/`6081` or to hold the display access code.
* **Listener ports come from the kernel, then get checked**: each relay binds `0.0.0.0:0` and rejects an answer that collides with a logical app port, a display port, an existing route, or a sibling — the dev server about to want `5173` is exactly the process that has not started yet. The accepted socket is held before publication, and a resume prefers the port the routes already name so a rebuilt relay usually keeps its URL.
* **The transaction grew a second half**: desired listeners ready → route update → metadata commit → obsolete relays stopped. Committed metadata now records the Sandbox identity, the `{logicalPort, relayPort, label}` mappings, and the applied relay-port set; the pending record carries complete previous and desired copies of both. Recovery commits only an exactly-matching desired state, verified against actual routes *and* PID/start-time-checked processes, and a cheap attach reuses a mapping only on that same evidence. The raw-port record shape is replaced, not supported alongside: an older record reads as absent and takes the full provisioning path.
* **Before the app listens, the URL answers**: a bounded `502` within three seconds, one attempt, generic body under 256 bytes, no internal paths. The same URL serves the app the moment the dev server starts — no second route update and no new URL to copy.
